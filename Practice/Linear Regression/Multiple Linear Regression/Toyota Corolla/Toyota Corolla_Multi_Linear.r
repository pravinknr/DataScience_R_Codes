...
corolla_model <- lm(corolla$Price~. , data = corolla)
...
final_data<- data.frame(corolla$Price, pred, Error = corolla_model$residuals)
view(final_data)
...
corolla_log_model<- lm(log(corolla$Price)~. , data = corolla)
summary(corolla_log_model)
pred_log <- predict(corolla_log_model)
data.frame(log(corolla$Price), pred_log, corolla_log_model$residuals)
